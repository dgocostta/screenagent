          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-4">
               <div className="flex flex-col items-end">
                 ...
                 <span className="text-[9px] text-zinc-500 font-bold uppercase">Super Admin</span>
               </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-4">
                ...
                <span className="bg-green-500/10 text-green-500 text-[9px] font-black px-2.5 py-1 rounded-md border border-green-500/20 uppercase tracking-widest">Active</span>
              </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="px-6 py-3 bg-[#FF6B00] rounded-xl text-[11px] font-black text-white uppercase italic tracking-widest shadow-2xl shadow-orange-600/40">+ New Agent</button>
            </div>
          </div>
          <div className="bg-[#121214] border border-white/5 rounded-3xl p-8 shadow-2xl space-y-8 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-6">
               {metrics.map((m) => (
                 <div key={m.label} className="bg-black/20 border border-white/5 p-5 rounded-2xl space-y-5 hover:border-[#FF6B00]/40 transition-all">
                    ...
                    <div className="flex items-baseline gap-3">
                       ...
                       <span className="text-[10px] font-black text-green-500 uppercase italic tracking-tighter">{m.change}</span>
                    </div>
                    <div className="h-4 bg-zinc-800 rounded-full overflow-hidden"><div className="h-full w-2/3 bg-[#FF6B00] animate-pulse shadow-[0_0_10px_#FF6B00]" /></div>
                 </div>
               ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                 ...
                 <div className="overflow-x-auto">
                    <table className="w-full text-left">
                       <thead className="bg-black/20 text-[10px] font-black text-zinc-700 uppercase tracking-widest">
                          <tr><th className="px-8 py-4 text-zinc-500">Product</th><th className="px-6 py-4 text-zinc-500">Price</th><th className="px-6 py-4 text-zinc-500">Status</th></tr>
                       </thead>
                          ...
                          {catalogItems.map(item => (
                            <tr key={item.id} className="hover:bg-white/[0.015] transition-colors">
                               <td className="px-8 py-5 flex items-center gap-4">
                                  <img src={item.image} className="w-12 h-12 rounded-xl object-cover shadow-2xl" alt="product" />
                                  <div className="flex flex-col"><span className="text-zinc-100 font-black">{item.name}</span><span className="text-[9px] text-zinc-700 uppercase tracking-tighter">SKU: {item.sku}</span></div>
                               </td>
              ...
              <div className="lg:col-span-2 bg-[#121214] border border-white/5 rounded-3xl overflow-hidden shadow-2xl flex flex-col items-center justify-center p-10 space-y-10">
                 <div className="flex flex-col items-center gap-3">
                    <div className="w-10 h-10 bg-[#FF6B00] rounded-xl flex items-center justify-center shadow-2xl rotate-6 shadow-orange-600/40"><span className="text-[15px] font-black italic text-white -rotate-6">M</span></div>
                 ...
                 <button className="bg-[#FF6B00] text-white text-[11px] font-black px-12 py-4 rounded-full shadow-[0_0_40px_rgba(255,107,0,0.6)] uppercase italic tracking-widest border border-white/10 hover:scale-105 transition-transform duration-300">Explore Collection</button>
              </div>
          </div>
        </div>
      </div>
